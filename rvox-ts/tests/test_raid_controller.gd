...
func test_strong_garrison_defends_small_raid() -> void:
	...
	var outcome := RaidController.resolve(5, true, 3)
...
func test_unprepared_settlement_falls_to_big_wave() -> void:
	...
	var outcome := RaidController.resolve(0, false, 12)
...
func test_more_swordsmen_flip_the_outcome() -> void:
	...
	var size := 10
	var few := RaidController.resolve(1, false, size)
	var many := RaidController.resolve(8, true, size)
